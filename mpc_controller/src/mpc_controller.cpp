                 ...
                 const double &steer_ratio)
{
    ...
    //When the control time domain length is 25, there are 24 control increments in total.
    // The control amount at the first time point is the same as the control increment.
...
Type     :
Function :
Input(s) :1. fg: where the cost function and vehicle model/contraints is defined; stored the relationship between vars
          2. vars: this vector contains all variables used by the cost function 
                   and model: [x,y,psi,v,cte,e_psi][front_wheel_angle,a](state & actuators) stored the vars
Retrun(s):
...
void FG_eval::operator()(ADvector &fg, ADvector &vars)
{
    ...
    // Objective term 2
    // Avoid to actuate as much as possible, minimize the use of actuators.
    ...
    // Objective term 3
    // Enforce actuators smoothness in change, minimize the value gap between sequential actuation.
    ...
    for (size_t t = 1; t < Np; t++) // prediction model
    {
        // Values at time (t),When entering this loop for the first time,
        // x_0... is passed in the real value of the system state given to MPC, which is used as the initial condition for MPC solution.
        ...
        //assumed that the lateral velocity will not change significantly due to the lateral acceleration during this period.
                                          ...
                                          const double &steer_ratio)
{
    ...
    // Set the initial variable values and bounds of value
    vars[x_start] = x;
    vars[y_start] = y;
    vars[psi_start] = psi;
